...
def test_workflow_filters_before_upload():
    ...
    configuration = yaml.safe_load((PROJECT_ROOT / initialize["with"]["config-file"]).read_text(encoding="utf-8"))
    assert configuration["queries"] == [{"uses": "security-extended"}]
    assert configuration["packs"] == ["codeql/python-queries:AlertSuppression.ql"]
    assert configuration["query-filters"] == [{"exclude": {"id": "py/request-without-cert-validation"}}]
