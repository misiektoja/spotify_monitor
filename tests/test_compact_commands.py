...
monitor = importlib.import_module("spotify_monitor")
...
def test_display_prefix_is_short(monkeypatch, host, executable, method):
    ...
    monkeypatch.setattr(sys, "argv", ["/private/long/tools/spotify_monitor.py" if method == "manual" else "/private/long/bin/spotify_monitor"])
    monkeypatch.setattr(monitor, "__file__", "/private/long/tools/spotify_monitor.py")
    ...
    expected = executable + " spotify_monitor.py --setup" if method == "manual" else "spotify_monitor --setup"
...
def test_help_does_not_display_runtime_paths():
    ...
    assert "spotify_monitor.py --setup" in result.stdout
