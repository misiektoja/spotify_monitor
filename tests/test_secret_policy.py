...
@pytest.mark.parametrize("destination_value", [None, "", "synthetic-destination"])
# Applies the same destination rule to Spotify's separate scrobble-health setup mode
def test_scrobble_setup_rechecks_destination(monitor, tmp_path, monkeypatch, destination_value):
    old, new = tmp_path / "old.env", tmp_path / "new.env"
    old.write_text("LASTFM_API_KEY=synthetic-old\n", encoding="utf-8")
    if destination_value is not None:
        new.write_text(f'LASTFM_API_KEY="{destination_value}"\n', encoding="utf-8")
    state = setup_state(monitor, tmp_path / "monitor.conf", old, mode="scrobble")
    state.secret_updates["LASTFM_API_KEY"] = "synthetic-pending"
    move_destination(monitor, state, new, monkeypatch, mode="scrobble")
    actual = save_private(monitor, state, new)
    assert actual["LASTFM_API_KEY"] == ("synthetic-pending" if destination_value is None else destination_value)
