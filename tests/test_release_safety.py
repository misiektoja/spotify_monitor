...
# Oversized saved timestamps follow the documented field-reset path
def test_scrobble_health_oversized_timestamp_resets_field(tmp_path, capsys):
    path = tmp_path / "health.json"
    path.write_text(json.dumps({"status": "broken", "broken_since": 10 ** 400, "last_notification_at": 1700000000}), encoding="utf-8")
    state = monitor.load_scrobble_health_state(path)
    assert state["broken_since"] == 0
    assert state["last_notification_at"] == 1700000000
    output = capsys.readouterr().out
    assert "broken_since" in output and "zero" in output
