...
- **IMPROVE:** **The profile picker shows which profile is signed in** - `--import-browser-cookie` marks every profile that **holds a current Spotify login** and preselects it when only one does, so Enter accepts it. Profiles are tagged with the install they came from, such as Snap or Flatpak. Two installs sharing a directory name are told apart by their cookie file. Guided setup reports how many profiles each browser has and how many hold a login, so you can pick a browser before opening the list
...
- **BUGFIX:** **Network failures point at the right page** - A timed-out or unreachable Spotify request ended with a **`Guide:`** link to the **Debugging Tools** page, which covers token utilities and says nothing about connection problems, and told you to run `--doctor --debug` for a check the monitor retries on its own. A Spotify server error sent you to Doctor for an outage Doctor cannot fix. Those reports now link to the new **Connection Problems** section, which explains the automatic retries and what to check if the failure continues. The file descriptor limit has its own section and an unreadable file links to the configuration page
