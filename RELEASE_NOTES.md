...
- **NEW:** Added a file-safe **`--doctor` self-check** for configuration, secrets, Spotify authentication, connectivity, target visibility and notifications. Interactive runs offer separate delivery tests for email and webhook channels. Each approved test sends one real message while non-interactive runs remain message-free. Common failures now include a clear **`To fix:` next step** instead of only a technical error
