...
- **NEW:** Added **Spotify-to-Last.fm scrobble health monitoring** to address Spotify's six-month re-authorization requirement. This can disconnect Spotify Scrobbling while Last.fm currently warns only through a website banner without sending an email alert. The monitor includes explicit selection through `--monitor-mode scrobble_health`, complete file-free runtime options, a focused setup wizard with Last.fm API account guidance plus duration-aware timer entry, separate default config and dotenv files, standalone hidden `LASTFM_API_KEY` entry through `--set-lastfm-credentials`, scoped recent-play access, conservative five-play outage detection, repeat reminders and recovery alerts
